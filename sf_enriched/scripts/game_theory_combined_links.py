...
class SumoSim():
    SUMOBIN = 'sumo'
    SUMOCMD = [SUMOBIN, "-c", "../config/config_with_TLS_combined.sumocfg", 
               "--time-to-teleport", "300",
               "--weight-files", "../config/link_weights_combined_"+suffix+".xml", 
               "--weight-attribute", "effort", "-W", "true"]
    
    def __init__(self):
        
        self.network = sumolib.net.readNet('../network/SF_with_TLS_combined.net.xml')
    ...
    def generate_edge_weights(self):
        f = open('../config/link_weights_combined_'+suffix+'.xml','w')
        op = "<meandata>\n"
        f.write(op)
    ...
    def calculate_all(self):
        ...
        for i,gt in enumerate(self.gt):
            ...
            gt.gamma.to_csv('../results/combined/'+ suffix + '/gamma_v5_{}.csv'.format(i))
            gt.rho.to_csv('../results/combined/'+ suffix + '/rho_v5_{}.csv'.format(i))
            gt.tau.to_csv('../results/combined/'+ suffix + '/tau_v5_{}.csv'.format(i))
            gt.densities.to_csv('../results/combined/'+ suffix + '/densities_v5_{}.csv'.format(i))
            gt.vulnerabilities.to_csv('../results/combined/'+ suffix + '/vulnerability_v3_{}.csv'.format(i))
...
if __name__ == "__main__":
    s = SumoSim()
    f = open('../results/combined/'+ suffix + '/'+ suffix + '_results.csv', 'w')
