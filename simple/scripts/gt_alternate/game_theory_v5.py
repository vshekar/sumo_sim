    ...
    def collect_interval_data(self):
        if self.step % self.delta == 0 and int(self.step/self.delta) > 0:
            ...
            for edge in self.edges:
                ...
                edgeID = edge.getID()
                self.densities[edgeID] /= self.delta
                if edgeID == '0to1':
                    print("Before dividing : {}".format(curr_gt.curr_tau[edgeID]))
                if self.iteration > 1:
                    curr_gt.curr_tau[edgeID] /= self.delta
                if edgeID == '0to1':
                    print("After dividing : {}".format(curr_gt.curr_tau[edgeID]))
    ...
    def reroute_vehicles(self):
        if self.iteration > 1:
            ...
            for edge in self.edges:
                ...
                rho = self.gt[interval].curr_rho[edgeID]
                beta = 1
                fft = traci.edge.getTraveltime(edgeID) 
                tt =  fft*(1-rho) + rho*beta*len(self.edges)*fft
                self.gt[interval].curr_tau[edgeID] += tt
                if edgeID == '0to1' and self.step%self.delta == 0:
                    print(self.gt[interval].curr_tau[edgeID])
    ...
    def calculate_all(self):
        ...
        for i,gt in enumerate(self.gt):
            ...
            if self.iteration ==1:
                gt.calc_s_expected()
                gt.calc_edge_cost()
            for k in gt.curr_tau.keys():
                gt.curr_tau[k] = gt.curr_tau[k]/self.delta
            print(gt.curr_tau)
            gt.calc_sys_vul()
