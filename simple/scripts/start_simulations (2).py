...
def generate_additional(edge,G,interval,intervals,suffix):
    ...
    lines = ('<additional>\n'
                            ...
                            '<edgeData id="em1" file="../output/edgeData'+ suffix +'.xml" freq="1" type="emissions" excludeEmpty="true"/>\n'
...
def start(G):
    ...
    for edge in vul_edges:
        for interval in intervals:
            ...
            print suffix +'\n'
            sumoProcess = subprocess.Popen(['sumo', "-c", "../config/config"+suffix+".sumocfg",'--vtk-output','../output/visualizations/sim'+suffix], stdout=sys.stdout, stderr=sys.stderr)
            #'--full-output','../output/full_op'+suffix]
            sumoProcess.wait()
            print "\n"
